...
SdpLookupChannel(SdpTaskParams &htp)
{
    ...
    const bdaddr_t bt_src = {0,0,0,0,0,0};
   	const bdaddr_t bt_dst = {0,0,0,0xff,0xff,0xff}; 

	//sdp = sdp_connect(BDADDR_ANY, &htp.m_bdaddr, SDP_RETRY_IF_BUSY);
	sdp = sdp_connect(&bt_src, &htp.m_bdaddr, SDP_RETRY_IF_BUSY);
	if (sdp == NULL) {
		return errno;
	}
...
const      bdaddr_t bt_dst = {0,0,0,0xff,0xff,0xff};
	...
	m_sdp = sdp_connect(&bt_src, &bt_dst, SDP_RETRY_IF_BUSY);
	if (m_sdp == NULL) {
